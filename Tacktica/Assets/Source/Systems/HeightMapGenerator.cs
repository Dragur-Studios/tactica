...
{
    public static HashSet<KeyValuePair<Vector2Int, float>> Perlin(HexWorldGenerator layout, int seed, Vector2Int offset, float scale, int octaves, float persistance, float lacunarity)
    {
        ...
        for (int x = -layout.range; x < layout.range; x++)
        {
            for (int y = -layout.range; y < layout.range; y++)
            {
                ...
                for (int i = 0; i < octaves; i++)
                {
                    float sampleX =  ((offset.x + (x -halfWidth)) / scale + octaveOffsets[i].x) * frequency;
                    float sampleY =  ((offset.y + (y - halfHeight)) / scale - octaveOffsets[i].y) * frequency;

                    float noiseValue = Mathf.PerlinNoise(sampleX, sampleY) * 2 - 1;
                    height += (noiseValue * amplitude) /** heightMultiplier*/;
                ...
                heightmap.Add(new KeyValuePair<Vector2Int, float>(coord, height));//[coord] = height;   
            }
        ...
        for (int x = -layout.range; x < layout.range; x++)
        {
            for (int y = -layout.range; y < layout.range; y++)
            {
                ...
                var val = heightmap.First(h => h.Key == coord).Value;
                val = Mathf.InverseLerp(minNoiseHeight, maxNoiseHeight, val);
            }
        }
    ...
    //public static HashSet<KeyValuePair<Vector2Int, float>> DiamondSquare(HexWorldGenerator layout, int seed, int mapSize, float roughness)
    //{
    //    HashSet<KeyValuePair<Vector2Int, float>> heightmap = new HashSet<KeyValuePair<Vector2Int, float>>();
    ...
    //        {
    //            var coord = new Vector2Int(x, y);
    //            heightmap.Add(new KeyValuePair<Vector2Int, float>(coord, 0));
    //        } 
    //    }
    ...
    //    var c1 = heightmap.First(h => h.Key == bl).Value;
    //    c1 = prng.Next(minHeight, maxHeight);
